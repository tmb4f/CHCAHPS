...
--ALTER PROCEDURE [Rptg].[uspSrc_Dash_PatExp_CHCAHPS_SvcLine]
--AS
/**********************************************************************************************************************
WHAT: Stored procedure for Patient Experience Dashboard - Child HCAHPS (Inpatient) - By Service Line
WHO : Tom Burgan
WHEN: 4/17/2018
...
IF OBJECT_ID('tempdb..#CHCAHPS_SvcLine ') IS NOT NULL
DROP TABLE #CHCAHPS_SvcLine
...
--SELECT resp.DOMAIN AS resp_DOMAIN
--      ,resp.Domain_Goals AS resp_Domain_Goals
...
--ON resp.Domain_Goals = goals.DOMAIN
--ORDER BY resp.DOMAIN
...
LEFT OUTER JOIN
	(SELECT * FROM Rptg.CHCAHPS_Goals WHERE UNIT = 'All Units') goals -- THIS IS SERVICE LINE LEVEL, USE THE SERVICE-LINE SPECIFIC GOAL, DENOTED BY UNIT "ALL UNITS" (ALL UNITS W/I SERVICE LINE GET THE GOAL)
...
(
	 ...
	 FULL OUTER JOIN
	     ...
	     ON dis.day_date = #surveys_ch_ip_sl.DISDATE
	 LEFT OUTER JOIN
		 (SELECT * FROM Rptg.CHCAHPS_Goals WHERE SERVICE_LINE = 'All Service Lines') goals  -- CHANGE BASED ON GOALS FROM BUSH - CREATE NEW CHCAHPS_Goals
...
-- RESULTS
 ...
 SELECT  [Event_Type]
   ...
   ,CASE WHEN CLINIC IN
	...
	) AND SERVICE_LINE <> 'All Service Lines' THEN CLINIC + ' - ' + SERVICE_LINE ELSE CLINIC END AS CLINIC -- adds svc line to units that appear in multiple service lines
   ...
   ,[quarter_name]
   ,[month_short_name]
  INTO #CHCAHPS_SvcLine
  FROM #surveys_ch_ip3_sl

  SELECT *
  FROM #CHCAHPS_SvcLine
  WHERE #CHCAHPS_SvcLine.SURVEY_ID IS NOT NULL
  --AND VARNAME = 'CH_48'
  --AND #CHCAHPS_SvcLine.SERVICE_LINE = 'Womens and Childrens - Children'
  --AND Domain_Goals = 'Global Rating Item' AND QUESTION_TEXT_ALIAS = 'Rate hospital 0-10'
  AND Domain_Goals IN ('Cleanliness','Quietness')
