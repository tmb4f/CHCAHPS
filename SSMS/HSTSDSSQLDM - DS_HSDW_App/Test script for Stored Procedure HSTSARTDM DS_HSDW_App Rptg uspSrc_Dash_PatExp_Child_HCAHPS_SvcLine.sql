...
--ALTER PROCEDURE [Rptg].[uspSrc_Dash_PatExp_Child_HCAHPS_SvcLine]
--AS
/**********************************************************************************************************************
...
----WHERE loc_master_EPIC_DEPARTMENT_ID <> dept_DEPARTMENT_ID
--ORDER BY SURVEY_ID
--       , sk_Dim_PG_Question
...
(
	 ...
	 FULL OUTER JOIN
	     ...
	     ON dis.day_date = #surveys_ch_ip_sl.DISDATE
	 LEFT OUTER JOIN
		 (SELECT * FROM #GOALS WHERE SERVICE_LINE = 'All Service Lines') goals  -- CHANGE BASED ON GOALS FROM BUSH - CREATE NEW CHCAHPS_Goals
...
-- RESULTS
 ...
 SELECT  [Event_Type]
   ...
   ,CASE WHEN UNIT IN
	...
	) AND SERVICE_LINE <> 'All Service Lines' THEN UNIT + ' - ' + SERVICE_LINE ELSE UNIT END AS UNIT -- adds svc line to units that appear in multiple service lines
