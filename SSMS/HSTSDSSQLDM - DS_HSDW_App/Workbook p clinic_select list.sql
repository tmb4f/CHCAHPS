...
MODS: 	10/22/2018 - Created procedure
		11/5/2018 - Edit logic used to assign an Epic Department Id value to a survey; retain the CLINIC column as
		            the identifier for an inpatient unit
***********************************************************************************************************************/
...
SELECT
	...
	,dep.Clrt_DEPt_Ext_Nme AS dep_Clrt_DEPt_Ext_Nme
...
SELECT locations.EPIC_DEPARTMENT_EXTERNAL_NAME, locations.EPIC_DEPARTMENT_ID
FROM
(
	SELECT
		--responses.SERVICE_LINE
	 --  ,responses.CLINIC
	   responses.EPIC_DEPARTMENT_EXTERNAL_NAME
	...
	(
        ...
        --         END AS VARCHAR(500)) AS CLINIC
	        CAST(CASE WHEN loc_master.epic_department_name IS NOT NULL AND loc_master.epic_department_name <> 'Unknown' THEN loc_master.epic_department_name_external
	                  ELSE dept.Clrt_DEPt_Ext_Nme
                 END AS VARCHAR(500)) AS EPIC_DEPARTMENT_EXTERNAL_NAME
